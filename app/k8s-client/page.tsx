...
function podStatusColor(status: string): string {
  ...
  if (s.includes('crash') || s.includes('error') || s.includes('oom')) {
    return 'error';
  }
  return 'warning';
}
...
export default function K8sClientPage() {
  ...
  const [descState, setDescState] = useState<DescribeState>({
    open: false,
    title: '',
    content: '',
    loading: false,
  ...
  const fetchResources = useCallback(async (type: string, ns: string) => {
    setResLoading(true);
    setResError('');
    try {
      ...
      if (res.ok) {
        ...
        setResError(data.error ?? 'Failed to load');
        setResources([]);
      }
    } catch (e: unknown) {
  ...
  useEffect(() => {
    ...
    fetchResources(activeTab, debouncedNamespace);
  }, [activeTab, debouncedNamespace, fetchResources]);
  ...
  const handleDescribe = async (resourceType: string, name: string, ns: string) => {
    ...
    try {
      ...
      setDescState((prev) => ({
        ...prev,
        content: d.output ?? d.error ?? 'No output',
        loading: false,
      }));
    } catch (e: unknown) {
      const err = e as { message?: string };
            ...
            <Button
              ...
              onClick={() =>
                setLogsState({
                  ...
                  pod: name,
                  namespace: ns,
                  containers: (record.containers as string[]) ?? [],
                })
              }
            />
          </Tooltip>
            ...
            <Button
              ...
              onClick={() =>
                setScaleState({
                  ...
                  name,
                  namespace: ns,
                  currentReplicas: (record.replicas as number) ?? 0,
                })
              }
            />
          </Tooltip>
  ...
  const COLUMNS: Record<string, TableColumnsType<ResourceRecord>> = {
    pods: [
      ...
      { title: 'Ready', dataIndex: 'ready', key: 'ready', width: 75 },
      {
        ...
        render: (v: unknown) => {
          ...
          return <Tag color={getColor()}>{n}</Tag>;
        },
      },
    ...
    configmaps: [
      ...
      nsCol,
      {
        ...
        key: 'dataCount',
        width: 90,
        render: (v: unknown) => `${v as number} key(s)`,
      },
      ageCol,
      { ...actionCol, width: 95 },
    ],
  ...
  return (
    <Layout style={{ minHeight: '100vh', backgroundColor: '#f0f2f5' }}>
      ...
      >
        <Title
          level={4}
          style={{ color: 'white', margin: 0, whiteSpace: 'nowrap', letterSpacing: 1 }}
        >
          ☸️ K8s Client
        </Title>
      ...
      <Content style={{ padding: '16px 24px' }}>
        ...
        {resError && (
          <Alert
            ...
            action={
              <Button size="small" onClick={() => fetchResources(activeTab, currentNamespace)}>
                Retry
              </Button>
            }
            closable
